...
class TipPopupUtilsKtFragmentTest {
    @get:Rule(order = 0)
    ...
    private fun withFragment(block: (Fragment) -> Unit) {
        ActivityScenario.launch<MainActivity>(Intent(context, MainActivity::class.java)).use { scenario ->
            ...
            scenario.onActivity { activity ->
                ...
                val fragment =
                    checkNotNull(navHost?.childFragmentManager?.primaryNavigationFragment) {
                        "NavHostFragment had no primary navigation fragment"
                    }
                block(fragment)
            }
            shadowOf(Looper.getMainLooper()).idle()
    ...
    @Test
    fun `showTipPopup with anchor view shows the popup`() {
        var popup: TipPopup? = null
        withFragment { fragment ->
            fragment.showTipPopup(
                message = "hello",
                getAnchor = { fragment.requireView().findViewById<View>(R.id.fabIcons) },
                onCreate = { popup = this },
            )
            shadowOf(Looper.getMainLooper()).idleFor(2, TimeUnit.SECONDS)
            popup?.isShowing shouldBe true
        }
    }

    @Test
    fun `showTipPopup with null anchor does not create a popup`() {
        var popup: TipPopup? = null
        withFragment { fragment ->
            fragment.showTipPopup(
                message = "hello",
